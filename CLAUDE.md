...
- `ShieldMeter` (GuiObject, `FuelMeter` still accepted): now the **shield meter**. Shown only while combat is active for the local player. Children: `BarBG.Fill` (sized 0..1 by `CombatController` to reflect shield charge) and `BarBG.EmptyOverlay` (visible while the shield is on cooldown).
